...
export function init_local() {
	expansions()
		.then((data) =>
			setStore.set(
				data.sort((a, b) => (Date.parse(a.releaseDate) > Date.parse(b.releaseDate) ? -1 : 1))
			)
		)
...
export function getEbayCardPrices(card: Card): Promise<EbayPrice[]> {
	...
	return new Promise<EbayPrice[]>((reslove, reject) => {
		let url = new URL(`${baseURL}/pokemon/card/price/ebay/${encodeURIComponent(card.cardId)}`);
...
export function getPriceChartData(card: Card): Promise<ChartData[]> {
	return new Promise<ChartData[]>(async (resolve, _) => {
		let data = new Array<ChartData>();
		if (card.cardId === '') resolve(data);
		let tcgp: TcgpPrice[] = [];
		let ebay: EbayPrice[] = [];
		try {
			tcgp = await getTcgpPrices(card.idTCGP);
			ebay = await getEbayCardPrices(card);
		} catch (err) {
			console.log(err);
			resolve(data);
		}
