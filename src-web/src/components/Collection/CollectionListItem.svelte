...
<tr>
    ...
    <td>
        ...
        <span class="badge badge-ghost badge-sm" style="background-color: {tag.color}; color: {getTextColorBgContrast(tag.color)}">{tag.name}</span>
        {/each}
    </td>
    <td>
        <CountControl class="w-64" bind:count={card.count} on:change={() => addCardCollection(card)}/>
    </td>
    <td>
