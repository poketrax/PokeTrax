...
<div class="input-group h-12 z-20 w-fit">
	<span>{label}</span>
	<div class="input dropdown pl-0 pr-1 {clazz}">
		<span
			id={`${label}-dropdown`}
			class="h-12 bg-base-100"
			tabindex="0"
			on:click={() => sortOptions(searchTerm)}
		>
			<Icon path={mdiMenuDown} class="h-6 w-6" />
		</span>
		<div
			class="dropdown-content shadow bg-base-100 rounded-box w-80 h-max max-h-96 overflow-y-scroll {modalClass}"
			tabindex="0"
		>
			<input
				class="input input-xs mt-2 ml-4 w-72 input-bordered"
				placeholder="Search"
				bind:value={searchTerm}
				on:change={() => sortOptions(searchTerm)}
			/>
			<div class="h-2" />
